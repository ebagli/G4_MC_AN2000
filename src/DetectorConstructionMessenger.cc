...
:fTarget(mpga){
    ...
    fXtalECAlCmd = new G4UIcmdWithAString("/xtal/setECAl",
                                         this);
    fXtalECAlCmd->SetGuidance("Set crystal EC for Al.");
    fXtalECAlCmd->SetParameterName("xECAl",true);
    fXtalECAlCmd->SetDefaultValue("data/Si220pl");
...
void DetectorConstructionMessenger::SetNewValue(
    ...
    if(command==fXtalECAlCmd ){
        fTarget->SetECAl(newValue);
    }
}
